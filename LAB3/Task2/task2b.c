...
void fix_file() {
    printf("Please enter offset and size of virus signature: ");
    int offset;
    int virus_size;
    fscanf(stdin, "%d %d", &offset, &virus_size);
    kill_virus(infected_filename, offset, virus_size);
}
...
void kill_virus(char *fileName, int signitureOffset, int signitureSize) {
    ...
    fseek(fp3, signitureOffset, SEEK_SET);
    char bits[signitureSize];
    for (int i = 0; i < signitureSize; ++i) {
        bits[i] = 0x90;
    }
    fwrite(bits, 1, (size_t) signitureSize, fp3);
    fclose(fp3);
}
